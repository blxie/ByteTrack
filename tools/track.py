...
def main(exp, args, num_gpu):
    ...
    # TODO: 查看 get_model() 函数的具体执行方式
    # 这里的 model 是包括了 BYTE 的 ByteTrack 还是仅仅 YOLOX
    model = exp.get_model()
    ...
    # TOTO: 个人理解，融合 YOLOX 和 BYTE 完成 ByteTrack？
    if args.fuse:
    ...
    *_, summary = evaluator.evaluate(model, is_distributed, args.fp16,
    ...
    if args.mot20:
        gtfiles = glob.glob(
            os.path.join('datasets/MOT20/train',
                         '*/gt/gt{}.txt'.format(gt_type)))
    else:
        gtfiles = glob.glob(
            os.path.join('datasets/mot/train',
                         '*/gt/gt{}.txt'.format(gt_type)))
