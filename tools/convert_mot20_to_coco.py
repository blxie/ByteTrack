...
SPLITS = ['train_half', 'val_half', 'train',
          'test']  # --> split training data to train_half and val_half.
HALF_VIDEO = True
    ...
    for split in SPLITS:
        ...
        for seq in sorted(seqs):
            ...
            for i in range(num_images):
                ...
                image_info = {
                    'file_name':
                    '{}/img1/{:06d}.jpg'.format(seq, i + 1),  # image name.
                    'id':
                    image_cnt + i +
                    1,  # image number in the entire training set.
                    'frame_id':
                    i + 1 - image_range[
                        0],  # image number in the video sequence, starting from 1.
                    'prev_image_id':
                    image_cnt + i if i > 0 else
                    -1,  # image number in the entire training set.
                    'next_image_id':
            ...
            if split != 'test':
                ...
                if CREATE_SPLITTED_DET and ('half' in split):
                    ...
                    for o in dets_out:
                        dout.write(
                            '{:d},{:d},{:.1f},{:.1f},{:.1f},{:.1f},{:.6f}\n'.
                            format(int(o[0]), int(o[1]), float(o[2]),
                ...
                for i in range(anns.shape[0]):
                    ...
                    if frame_id - 1 < image_range[
                            0] or frame_id - 1 > image_range[1]:
                        continue
