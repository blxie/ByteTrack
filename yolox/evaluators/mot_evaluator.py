...
class MOTEvaluator:
    ...
    def __init__(self, args, dataloader, img_size, confthre, nmsthre,
                 num_classes):
        """
        Args:
    ...
    def evaluate(self,
        ...
        # TRACED 结合 BYTE 的追踪模型，Combining BYTE with other detectors
        # from yolox.tracker.byte_tracker import BYTETracker
        ...
        for cur_iter, (imgs, _, info_imgs,
            ...
            with torch.no_grad():
                ...
                # TRACED 关键点，参考注释怎么将 BYTE 融合到其他的检测器中
                # TODO: model ~ YOLOX 检测器的输出是什么格式？
                outputs = model(imgs)
                if decoder is not None:
                    outputs = decoder(outputs, dtype=outputs.type())

                # TRACED 这里的后处理起什么作用？根据 YOLOX 的 forward() 输出的 outputs 格式全是一堆损失函数和置信度
                # 坐标信息是怎么得到的？
            ...
            if outputs[0] is not None:
                online_targets = tracker.update(outputs[0], info_imgs,
                                                self.img_size)
                online_tlwhs = []
                online_ids = []
                online_scores = []
                for t in online_targets:
                    ...
                    if tlwh[2] * tlwh[
                            3] > self.args.min_box_area and not vertical:
                        online_tlwhs.append(tlwh)
                        online_ids.append(tid)
                        online_scores.append(t.score)
                # save results
                results.append(
                    (frame_id, online_tlwhs, online_ids, online_scores))
        ...
        for cur_iter, (imgs, _, info_imgs,
            ...
            with torch.no_grad():
                ...
                if video_name not in video_names:
                    video_names[video_id] = video_name
                if frame_id == 1:
                    tracker = DeepSort(model_folder,
                                       min_confidence=self.args.track_thresh)
                    if len(results) != 0:
                        result_filename = os.path.join(
                            result_folder,
                            '{}.txt'.format(video_names[video_id - 1]))
    ...
    def evaluate_motdt(self,
        ...
        tracker = OnlineTracker(model_folder,
                                min_cls_score=self.args.track_thresh)
        for cur_iter, (imgs, _, info_imgs,
                       ids) in enumerate(progress_bar(self.dataloader)):
            with torch.no_grad():
                # init tracker
